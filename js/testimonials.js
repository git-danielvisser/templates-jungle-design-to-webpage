import { screenSizeTypes, getScreenSizeType, debounceDecorator } from "./utils.js";
...
class Testimonials {
    constructor(element) {
        ...
        this.currentSlide = 0;
        this.testimonialsAmount = this.element.querySelectorAll(`.${TESTIMONIAL_CLASS}`).length;
        this._buildNavigation();

        // Adapt to screen size change 
        this._adaptToScreenSizeChange = debounceDecorator(this._adaptToScreenSizeChange, 100);
    ...
    moveToSlide(i) {
        ...
        const rowEle = this.element.querySelector(`.${ROW_CLASS}`);
        const translateX = this.currentSlide * -(100 + TESTIMONIAL_MARGIN_RIGHT_PERCENTAGE);
    ...
    _buildNavigation(){
        ...
        this.slidesAmount = getSlidesAmount(this.testimonialsAmount, screenSizeType);
